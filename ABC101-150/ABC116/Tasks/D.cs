...
namespace Tasks
{
    public class D
    {
        ...
        public static void Solve()
        {
            ...
            var Sushi = new (int t, long d)[N];
            for (var i = 0; i < N; i++)
            {
                var (t, d) = Scanner.Scan<int, long>();
                Sushi[i] = (t - 1, d);
            }
            Array.Sort(Sushi, (x, y) => y.d.CompareTo(x.d));

            var queue = new PriorityQueue<long>();
            var usedType = new bool[N];
            var sum = 0L;
            var typeCount = 0L;
            foreach (var (t, d) in Sushi.Take(K))
            {
                if (usedType[t]) queue.Enqueue(d);
                else typeCount++;
                usedType[t] = true;
                sum += d;
            }
            var answer = sum + typeCount * typeCount;
            foreach (var (t, d) in Sushi.Skip(K))
            {
                if (!queue.Any()) break;
                ...
                usedType[t] = true;
                sum += d - queue.Dequeue();
                answer = Math.Max(answer, sum + typeCount * typeCount);
            }
