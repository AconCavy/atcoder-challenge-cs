...
namespace Tasks
{
    public class E
    {
        ...
        public static void Solve()
        {
            ...
            var events = new List<(long T, long E, long X)>();
            for (var i = 0; i < N; i++)
            {
                var (S, T, X) = Scanner.Scan<long, long, long>();
                events.Add((S - X, 1, X));
                events.Add((T - X, -1, X));
            }
            ...
            var qs = new PriorityQueue<long>();
            var qe = new PriorityQueue<long>();
            foreach (var (_, e, x) in events.OrderBy(x => x.T).ThenByDescending(x => x.X))
            {
                if (e == 1) qs.Enqueue(x);
                else if (e == -1)
                {
                    qe.Enqueue(x);
                    while (qe.TryPeek(out var a) && qs.TryPeek(out var b) && a == b)
                    {
                        qe.Dequeue();
                        qs.Dequeue();
                    }
                }
                else Console.WriteLine(qs.TryPeek(out var answer) ? answer : -1);
            }
        }
