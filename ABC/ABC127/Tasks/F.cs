...
namespace Tasks
{
    public class F
    {
        ...
        public static void Solve()
        {
            ...
            var (sum, lSum, rSum) = (0L, 0L, 0L);
            var lq = new PriorityQueue<long>(new[] { long.MinValue }, (x, y) => y.CompareTo(x));
            var rq = new PriorityQueue<long>(new[] { long.MaxValue });
            while (Q-- > 0)
            {
                ...
                if (query[0] == 1)
                {
                    var (a, b) = (query[1], query[2]);
                    sum += b;
                    if (c < a) { rq.Enqueue(a); rSum += a; }
                    else { lq.Enqueue(a); lSum += a; }

                    if (rq.Count - lq.Count > 0)
                    {
                        var r = rq.Dequeue();
                        lq.Enqueue(r);
                        rSum -= r;
                ...
                var answer = rSum - lSum + sum;
                if (rq.Count < lq.Count) answer += c;
