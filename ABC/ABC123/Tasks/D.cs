...
namespace Tasks
{
    public class D
    {
        ...
        {
            var (X, Y, Z, K) = Scanner.Scan<int, int, int, int>();
            var A = Scanner.ScanEnumerable<long>().ToArray();
            var B = Scanner.ScanEnumerable<long>().ToArray();
            var C = Scanner.ScanEnumerable<long>().ToArray();

            Comparison<long> descending = (x, y) => y.CompareTo(x);
            Array.Sort(A, descending);
            Array.Sort(B, descending);
            Array.Sort(C, descending);
            var queue = new PriorityQueue<(long Value, int, int, int)>((x, y) => y.Value.CompareTo(x.Value));
            queue.Enqueue((A[0] + B[0] + C[0], 0, 0, 0));
            var used = new bool[X, Y, Z];
            used[0, 0, 0] = true;

            while (K-- > 0)
            {
                var (answer, i, j, k) = queue.Dequeue();
                Console.WriteLine(answer);
                foreach (var (di, dj, dk) in new[] { (1, 0, 0), (0, 1, 0), (0, 0, 1) })
                {
                    var (ni, nj, nk) = (i + di, j + dj, k + dk);
                    if (ni < X && nj < Y && nk < Z && !used[ni, nj, nk])
                    {
                        queue.Enqueue((A[ni] + B[nj] + C[nk], ni, nj, nk));
                        used[ni, nj, nk] = true;
                    }
                }
            }
